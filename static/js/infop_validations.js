...
function saveInformacionProponente(proyectoId) {
    const data = new FormData(document.getElementById('informacion_proponente_form'));
    
    fetch(`/proyecto/${proyectoId}/informacion_proponente/`, {
        method: 'POST',
        body: data,
    })
    ...
    .then(data => {
        if (data.success) {
            // Mueve al siguiente paso del stepper o muestra un mensaje de éxito
            moveToNextStep();
        } else {
